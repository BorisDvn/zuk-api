...
//			return Announcement2AnnouncementReadTO.apply(adminRepository.findByEmail(email).get().getAnnouncements());
    ...
    public AnnouncementReadTO addAnnouncement(AnnouncementWriteTO announcement, List<MultipartFile> files) {
        ...
        if (announcement.getCreatorId() != null) {
            ...
            switch (announcement.getCreatorStatus().toString()) { // todo trouver un moyen pour recuperer l'user ici et exploiter
                case "SEEKER":
                ...
                case "HELPER":
                    ...
                    newAnnouncement.setHelper(helperService.findHelper(announcement.getCreatorId())); // because of casting
                    break;
                case "ADMIN":
